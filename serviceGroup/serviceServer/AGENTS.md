...
- **打包范围 = 单服务名 `serviceServer-rust`**（2026-09-22 修）：`serviceServer-legacy/` 自 U8 摘除 `config.yaml` 条目后**已不是服务**，其模板/静态/数据层助手/CommonTools 经 `make_deploy_pack.py::EXTRA_ASSET_PREFIXES` **并回 rust 的资产域** ⇒ `--only serviceServer-rust` 即覆盖全包（实测 72 文件，其中 legacy 55）。**旧的双服务名 `--only serviceServer-rust,serviceServer-legacy` 已弃用**（U8 后直接 `未知服务名` 报错 —— 这是 U8 遗留的静默漏发/打包失败缺口，已修）。
- **按机数据不入包（首次部署初始化清单）**：`config.json`（`config*` 前缀排除）、`db_creds.enc`(+`.bak`，按机凭据)、`templates.db`（`.gitignore *.db`；Rust 缺库时自建且「**已有模板不丢**」）三者**刻意不随包**，须人工放置 / 运行期自建；`oss_hosts.yaml`（hostID 登记表）与 `script.json`（启动序列）**随包覆盖**，以推送端为准。
