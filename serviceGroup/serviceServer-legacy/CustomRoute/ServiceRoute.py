...
# ==================== AI Benchmark 调度与 API ====================
from CommonTools.agent.speedTest import run_benchmark, save_benchmark, get_latest_benchmark, get_all_benchmarks, MODELS as BENCHMARK_MODELS, analyze_results
...
API_PROVIDERS = {
    'mdproxy': {
        ...
        'auth_token': 'sk-dO3EZ0tc0PeU92wRhkPUp60sukthYvaaN1BJ1DeKJNlAl9c3',
        'models': ['deepseek-v4-flash', 'deepseek-v4-pro',"kimi-k2.6", 'glm-5', 'glm-5.1', 'kimi-k2.5', 'MiniMax-M2.7', 'doubao-seed-2.0-code', 'doubao-seed-2.0-pro', 'step3.5-flash-fp8', 'gpt-5.5', 'claude-opus-4-7'],
        'default_model': 'glm-5',
    ...
    'deepseek': {
        ...
        'auth_token': 'sk-4504b4ccb6384abfb7735762ccf3952e',
        'models': ['deepseek-v4-flash', 'deepseek-v4-pro', 'deepseek-chat'],
        'default_model': 'deepseek-v4-flash'
    }
}

def get_current_api_provider():
    """获取当前 API 来源配置"""
    ...
    try:
        if os.path.exists(settings_path):
            ...
            # 根据base_url判断来源
            if base_url == API_PROVIDERS['deepseek']['base_url']:
                return 'deepseek'
            elif base_url == API_PROVIDERS['mdproxy']['base_url']:
                return 'mdproxy'
            # 默认返回 mdproxy
            return 'mdproxy'
    except:
...
_benchmark_start_time = None
_benchmark_timer = None

def _do_benchmark_and_save():
    """在线程中执行 benchmark 并保存结果。"""
    global _benchmark_running, _benchmark_start_time
    ...
    try:
        results = run_benchmark(return_results=True)
        analysis = analyze_results(results)
        save_benchmark(results, analysis)
        print(f"[Benchmark] 测试完成并保存, 时间: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    except Exception as e:
...
def _schedule_benchmark():
    """每2小时执行一次 benchmark 的调度器。"""
    return
    # global _benchmark_timer
    # # 在后台线程中执行首次 benchmark，避免阻塞服务启动
    # thread = threading.Thread(target=_do_benchmark_and_save)
    # thread.daemon = True
    # thread.start()
    # _benchmark_timer = threading.Timer(7200, _schedule_benchmark)  # 2小时 = 7200秒
    # _benchmark_timer.daemon = True
...
def ai_manager_page():
    """显示 AI 管理页面"""
    return render_template('AIManager.html')
...
def api_benchmark_latest():
    ...
    try:
        ...
        if data is None:
            return jsonify({'success': False, 'message': '暂无 benchmark 数据'})
        return jsonify({'success': True, 'data': data})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500

@app.route('/api/benchmark/all', methods=['GET'])
def api_benchmark_all():
    """获取所有 benchmark 数据"""
    try:
        data = get_all_benchmarks()
        return jsonify({'success': True, 'data': data})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500

@app.route('/api/benchmark/trigger', methods=['POST'])
def api_benchmark_trigger():
    """手动触发一次 benchmark 测试"""
    return jsonify({'success': False, 'message': '由于调用数限制，暂时关闭压测脚本。'})

    global _benchmark_running
    if _benchmark_running:
        return jsonify({'success': False, 'message': '测试正在执行中，请稍候'})
    thread = threading.Thread(target=_do_benchmark_and_save)
    thread.daemon = True
    thread.start()
    return jsonify({'success': True, 'message': 'benchmark 测试已触发'})

@app.route('/api/benchmark/status', methods=['GET'])
def api_benchmark_status():
    """获取 benchmark 当前运行状态"""
    elapsed = None
    if _benchmark_running and _benchmark_start_time:
        elapsed = round(time.time() - _benchmark_start_time, 1)
    return jsonify({'success': True, 'running': _benchmark_running, 'elapsed_seconds': elapsed})

@app.route('/api/claude/model', methods=['GET'])
def api_get_claude_model():
    """获取当前 Claude settings.json 中的配置信息"""
    try:
        settings_path = os.path.join(os.path.expanduser('~'), '.claude', 'settings.json')
        if not os.path.exists(settings_path):
            default_provider = 'mdproxy'
            default_models = API_PROVIDERS[default_provider]['models']
            return jsonify({
                'success': True,
                'current_model': API_PROVIDERS[default_provider]['default_model'],
        ...
        current_provider = get_current_api_provider()

        # 返回当前 provider 的模型列表
        provider_config = API_PROVIDERS.get(current_provider, API_PROVIDERS['mdproxy'])
...
@app.route('/api/claude/provider', methods=['GET'])
def api_get_providers():
    """获取所有可用的 API 来源"""
    return jsonify({
        'success': True,
        'providers': API_PROVIDERS,
        'current': get_current_api_provider()
    })

@app.route('/api/claude/provider', methods=['POST'])
def api_set_provider():
    ...
    try:
        ...
        if not provider_key:
            return jsonify({'success': False, 'message': '缺少 provider 参数'}), 400

        if provider_key not in API_PROVIDERS:
            return jsonify({'success': False, 'message': f'未知的 API 来源: {provider_key}'}), 400
        ...
        # 设置默认模型
        settings['env']['ANTHROPIC_MODEL'] = provider.get('default_model', provider['models'][0])

        # 设置小模型（如果配置中有）
        if provider.get('small_fast_model'):
            settings['env']['ANTHROPIC_SMALL_FAST_MODEL'] = provider['small_fast_model']
        ...
        return jsonify({
            'success': True,
            'message': f'已切换到 {provider["name"]}',
            'provider': provider_key,
            'models': provider['models'],
            'current_model': settings['env']['ANTHROPIC_MODEL']
        })
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500

@app.route('/api/claude/model', methods=['POST'])
def api_set_claude_model():
    """设置 Claude settings.json 中的 ANTHROPIC_MODEL 字段"""
    try:
        data = request.json
        new_model = data.get('model')
        if not new_model:
            return jsonify({'success': False, 'message': '缺少 model 参数'}), 400

        # 根据当前 provider 验证模型
        current_provider = get_current_api_provider()
        provider_config = API_PROVIDERS.get(current_provider, API_PROVIDERS['mdproxy'])
        valid_models = provider_config['models']

        # 也允许 Benchmark 测试的模型列表
        all_valid_models = list(set(valid_models + list(BENCHMARK_MODELS)))
        if new_model not in all_valid_models:
            return jsonify({'success': False, 'message': f'模型 {new_model} 不在可选列表中'}), 400
        ...
        return jsonify({'success': True, 'message': f'已将 ANTHROPIC_MODEL 切换为 {new_model}'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
...
AI_API_BASE_URL = "http://aiapi.tcy365.net:82"
CLAUDE_SETTINGS_PATH = os.path.join(os.path.expanduser('~'), '.claude', 'settings.json')

def get_claude_model():
    """获取 Claude settings.json 中的 ANTHROPIC_MODEL"""
    try:
        if os.path.exists(CLAUDE_SETTINGS_PATH):
            with open(CLAUDE_SETTINGS_PATH, 'r', encoding='utf-8') as f:
                settings = json.load(f)
            return settings.get('env', {}).get('ANTHROPIC_MODEL', None)
    except:
        pass
    return None

@app.route('/api/ai-proxy/<path:subpath>', methods=['GET', 'POST', 'PUT', 'DELETE', 'PATCH'])
def api_ai_proxy(subpath):
    """转发请求到 API 聚合平台"""
    try:
        # 构建目标 URL
        target_url = f"{AI_API_BASE_URL}/{subpath}"
        ...
        return jsonify({'success': False, 'message': '无法连接到目标服务器'}), 502
    except Exception as e:
        return jsonify({'success': False, 'message': f'转发请求失败: {str(e)}'}), 500
...
def rag_qa_page():
    """显示 RAG 知识问答页面"""
    return render_template('ragQA.html')

@app.route('/api/rag/status', methods=['GET'])
def api_rag_status():
    """获取 RAG 索引状态"""
    try:
        from CommonTools.ragKnowledge import get_engine
        engine = get_engine()
...
def api_rag_query():
    ...
    if not question:
        return Response(
            f"data: {json.dumps({'type': 'error', 'data': '请输入问题'})}\n\n",
    ...
    def generate():
        try:
            ...
            for event in engine.query_stream(question):
...
def api_rag_models():
    """获取所有嵌入模型信息"""
    try:
        from CommonTools.ragKnowledge.config import get_embedding_model_info
...
@app.route('/api/rag/model/switch', methods=['POST'])
def api_rag_model_switch():
    """切换嵌入模型"""
    try:
        data = request.json
        ...
        if not model_key:
            return jsonify({'success': False, 'message': '请指定模型'}), 400

        from CommonTools.ragKnowledge import get_engine
        engine = get_engine()
        result = engine.switch_embedding_model(model_key)
...
def api_rag_sync():
    """异步启动飞书同步和索引重建"""
    try:
        from CommonTools.ragKnowledge.scheduler import start_async_sync
        result = start_async_sync()
        return jsonify(result)
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500


@app.route('/api/rag/sync/status', methods=['GET'])
def api_rag_sync_status():
    """获取同步任务状态"""
    try:
        from CommonTools.ragKnowledge.scheduler import get_sync_status
