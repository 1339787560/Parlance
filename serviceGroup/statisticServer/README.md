...
### 费用计算（DeepSeek 峰谷计价，**人民币** / 百万 tokens）

| 模型 | 计费项 | 空闲时段 | 高峰(北京时间 9:00-12:00 / 14:00-18:00) |
|------|--------|------|------|
| Flash (`deepseek-flash`，旧名 `deepseek-v4-flash`) | 输入未命中 / 输入命中 / 输出 | ¥1 / ¥0.02 / ¥4 | ¥2 / ¥0.04 / ¥8 |
| Pro (`deepseek-v4-pro`) | 输入未命中 / 输入命中 / 输出 | ¥4.5 / ¥0.15 / ¥13.5 | ¥9 / ¥0.30 / ¥27 |

费用 = `(输入未命中 × 未命中单价 + 缓存命中 × 命中单价 + 输出 × 输出单价) / 1_000_000`，高峰时段整体 ×2。

> 刊例出处：`api-docs.deepseek.com/zh-cn/quick_start/pricing`（2026-09-10 核实，人民币刊例）。
> Flash 旧名 `deepseek-v4-flash` 对应模型已下线，请求由 DeepSeek-V4.1-Flash 提供服务并按 Flash 价计费。
> 北京时间 2026-09-14 12:00 起，`deepseek-v4-pro` 请求全部路由到 V4.1 Flash 并按 Flash 价计费（至 V4.1 Pro 上线）。
