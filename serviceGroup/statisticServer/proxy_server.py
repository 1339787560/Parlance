#!/usr/bin/env python3
"""
Transparent proxy: http://127.0.0.1 -> https://api.deepseek.com/anthropic
...
TARGET = os.environ.get("TARGET", "https://api.deepseek.com/anthropic")
...
async def record(d: dict):
    ...
    conn.execute("""
        ...
        (id, ts, session_id, model, prompt_tokens, completion_tokens,
         total_tokens, cache_hit_tokens, cache_miss_tokens, latency_ms, status)
        VALUES (?,?,?,?,?,?,?,?,?,?,?)
    """, (
...
if __name__ == "__main__":
    ...
    if not DEEPSEEK_API_KEY:
        ...
        print(f"Proxy http://{args.host}:{PORT} -> {TARGET}")
