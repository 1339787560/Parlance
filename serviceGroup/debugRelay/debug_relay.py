...
def _debug_index_expr(env: str = None) -> str:
    ...
    return (
        ...
        "if(g&&typeof g.registry==='function'){"
        "var reg=g.registry(" + (f"'{env}'" if env else "") + ")||{fns:{},refs:{}};"
        "var fns=reg.fns||{};"
        "Object.keys(fns).forEach(function(ns){Object.keys(fns[ns]).forEach(function(fn){"
